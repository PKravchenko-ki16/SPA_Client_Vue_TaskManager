 ...
 <li>         
   <div v-if="seeninfo">
     <h5>Подзадач не созданно(</h5>
   </div>
   <div v-if="seenfalse === false">
 ...
 export default {
  ...
  methods: {
    beckremove()
      ...
      restbefotask()
      {
        ...
        function sq(value,index,array) {
          if (value.id === vm.itt) {
            value.title = inputfilter.methods.filter(vm.titlebefotask);
            value.deadline = inputfilter.methods.filter(vm.datepickbefotask);
            value.priority = inputfilter.methods.filter(vm.pickedbefotask);
          }
        }
    ...
    seenbuttask()
      ...
      updatetask(Etask)
      {
           var params = 'Title=' + inputfilter.methods.filter(Etask.title) + 
					'&Deadline=' + Etask.deadline.toISOString() +
    ...
    delitetask(id)
      {
          ...
          function responceLoad()
          {
           if (xhr.readyState == 4) 
           {
             ...
             if (status == 200)
              {
                vm.Etasks.splice(id,1);
                vm.beckremove();
                window.location.reload();// надо убрать по хорошему!!!!! передать пораметр seen = false родителю
              } 
            else
             {
    ...
    Createnewtasktotask()
      ...
      gettasks()
        {
          var params = 'id_task=' + this.taskid +
          '&projectId=' + this.id_project;
